...
The invariance enforced by gradient reversal operates in the latent space of encoders, but the *metric* on that latent space---the Mahalanobis distance from Chapter 2---depends on a positive-definite precision matrix $\Sigma^{-1}$. Chapter 15 addresses the fundamental engineering question of how to *learn* this metric: the Cholesky parameterization $\Sigma^{-1} = LL^\top$ transforms the constrained optimization over SPD matrices into an unconstrained optimization over lower-triangular matrices. The log-diagonal variant removes the last remaining constraint (positive diagonals) and yields a parameterization that is fully unconstrained, numerically stable, and differentiable end-to-end.

The connection to gradient reversal is direct: the domain discriminator's latent space geometry is shaped by the metric, and the Cholesky parameterization provides the machinery for learning that metric while maintaining positive-definiteness. Chapter 15 develops this parameterization in full and connects it back to the SPD manifold geometry of Chapter 4.
